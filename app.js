...
function emailDashboard() {
  ...
  var hotel = _HOTEL_NAME || 'Pearl';
  var mt = monthTotals(CY, m);
  var qr = mt.qr || 0;
  var kg = Math.ceil(mt.kg || 0);
  var act = getActiveDays(CY, m);
  var tgt = loadTarget(CY, m);
  var tgtRev = tgt && tgt.revenue ? tgt.revenue : 0;
  var pct = tgtRev > 0 ? (qr/tgtRev*100).toFixed(1)+'%' : 'N/A';

  var lines = [
    hotel + ' Laundry - ' + mn + ' ' + CY + ' Dashboard Summary',
    '================================',
    'Monthly Revenue:  QR ' + fmtMoney(qr),
    'Target:           QR ' + (tgtRev > 0 ? fmtMoney(tgtRev) : 'Not set'),
    'vs Target:        ' + pct,
    'Total KG Washed:  ' + kg + ' kg',
    'Active Days:      ' + act + ' days',
    'Avg Daily Rev:    QR ' + (act > 0 ? fmtMoney(qr/act) : '0.00'),
    '================================',
    'Generated: ' + new Date().toLocaleString()
  ];
  var subj = encodeURIComponent(hotel + ' Laundry - ' + mn + ' ' + CY + ' Dashboard');
...
function emailMonthly() {
  ...
  var lines = [
    hotel + ' Laundry - ' + mn + ' ' + CY + ' Monthly Report',
    dsel !== 'ALL' ? ('Department: ' + dsel) : '',
    '================================',
    'Total Revenue: QR ' + fmtMoney(qr),
    'Total KG:      ' + kg + ' kg',
    '',
    'Department Breakdown:'
  ].filter(function(l){ return l !== null; });

  var deptsToShow = dsel === 'ALL' ? DEPT_KEYS : [dsel];
  deptsToShow.forEach(function(d) {
    ...
    MASTER[d].forEach(function(_, i) {
      for (var day = 1; day <= nd; day++) {
        ...
        if (v > 0) {
          dqr += v * ((mpr[d] && mpr[d][i] ? mpr[d][i][1] : null) || MASTER[d][i][1]);
          dkg += v * (MASTER[d][i][2] || 0);
        }
      }
    });
    if (dqr > 0) lines.push('  ' + d + ': QR ' + fmtMoney(dqr) + ' / ' + Math.ceil(dkg) + ' kg');
  });

  lines.push('================================');
  lines.push('Generated: ' + new Date().toLocaleString());

  var subj = encodeURIComponent(hotel + ' Laundry - ' + mn + ' ' + CY + ' Monthly Report');
  var body = encodeURIComponent(lines.join('\n'));
  window.location.href = 'mailto:?subject=' + subj + '&body=' + body;
}

function emailFinance() {
  ...
  var qr   = mt.qr || 0;
  var prevCarry = 0;
  try { prevCarry = parseFloat(localStorage.getItem('pearl_carry_' + CY + '_' + m) || '0') || 0; } catch(e) {}
  var total = prevCarry + qr;

  var lines = [
    hotel + ' Laundry - ' + mn + ' ' + CY + ' Finance Posting',
    '================================',
    'Carry Forward:      QR ' + fmtMoney(prevCarry),
    'Month Revenue:      QR ' + fmtMoney(qr),
    '--------------------------------',
    'TOTAL FOR POSTING:  QR ' + fmtMoney(total),
    '================================',
    'Generated: ' + new Date().toLocaleString()
  ];
  var subj = encodeURIComponent(hotel + ' Laundry - ' + mn + ' ' + CY + ' Finance Posting');
  var body = encodeURIComponent(lines.join('\n'));
  window.location.href = 'mailto:?subject=' + subj + '&body=' + body;
}
